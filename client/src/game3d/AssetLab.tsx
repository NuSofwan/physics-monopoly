...
export function AssetLab(): JSX.Element {
  ...
  useEffect(() => { const update = () => setCompact(window.innerWidth < 1100); window.addEventListener("resize",update); return () => window.removeEventListener("resize",update); },[]);
  ...
  return <main className="asset-lab min-h-screen p-5">
    ...
    {mode === "board" ? <Board3D state={budgetState} moveEvent={null} reducedMotion quality={labQuality} onUnavailable={fail} measureAssets/> : failed ? <p role="alert">WebGL ไม่พร้อมใช้งาน ใช้กระดานข้อความในเกมได้</p> : <div className="h-[68vh] min-h-[450px] overflow-hidden rounded-2xl border border-slate-200 shadow-xl" aria-label="ตัวอย่างโมเดลสามมิติ"><Canvas shadows camera={{ position: mode === "buildings" ? compact ? [0,10,22] : [0,7,13] : mode === "city" ? compact ? [9,10,15] : [8,6.5,10] : compact ? [0,12,17] : [0,7,12], fov:42 }} dpr={[1,1.5]}>
      <AssetMetrics/>
      <CameraControls reset={reset} onUnavailable={fail} initialPosition={mode === "buildings" ? compact ? [0,10,22] : [0,7,13] : mode === "city" ? compact ? [9,10,15] : [8,6.5,10] : compact ? [0,12,17] : [0,7,12]} initialTarget={mode === "buildings" ? [0,1.2,0] : mode === "city" ? [0,1,0] : [0,.7,0]} minDistance={mode === "buildings" ? 6 : 7}/><color attach="background" args={[mode === "city" ? SKY.fog : "#dce4e4"]} />{mode === "city" ? <fog attach="fog" args={[SKY.fog,30,72]}/> : null}<SkyAndEnvironment showSky={mode === "city"}/><SceneLights quality={previewQuality} extent={13}/><WaterMotion/>{previewQuality === "high" ? <PostEffects/> : null}<QualityContext.Provider value={previewQuality}>
