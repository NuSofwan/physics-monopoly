...
import { CircleGeometry, Euler, Group, Quaternion, Vector3 } from "three";
...
export function Board3D({ state, moveEvent, reducedMotion = false, quality = "low", onUnavailable, onRestored, measureAssets = false }: { state: GameState; moveEvent: MoveEvent | null; reducedMotion?: boolean; quality?: "low" | "medium"; onUnavailable: () => void; onRestored?: () => void; measureAssets?: boolean }): JSX.Element {
  ...
  return (
    <div className="board-frame relative h-[60vh] min-h-[360px] w-full" style={{ aspectRatio: "auto" }} aria-label={`กระดานสามมิติ${city.name}`}>
      ...
      <Canvas shadows={quality === "medium"} camera={{ position: [0, 17, 18], fov: 42 }} dpr={quality === "low" ? 1 : [1, 1.5]} fallback={<p>ไม่สามารถเปิด 3 มิติ กรุณาเลือกกระดานข้อความ</p>}>
        {measureAssets ? <AssetMetrics/> : null}
        <CameraControls reset={reset} onUnavailable={onUnavailable} onRestored={onRestored} followTile={follow&&!reducedMotion?state.players[state.currentPlayerIndex]?.tileIndex:undefined} />
        <color attach="background" args={[city.sky]} />
        <fog attach="fog" args={[city.sky, 20, 45]} />
        <ambientLight intensity={1.2} />
        <directionalLight castShadow intensity={1.4} position={[8, 14, 6]} shadow-mapSize={[1024, 1024]} />
...
function WalkingRing(): JSX.Element {
  // A continuous raised lane supports the second row of tokens outside tile
  // footprints. Top is just below tile tops, avoiding coplanar z-fighting.
  return <group>{[-1,1].map(side=><group key={side}>
    <mesh receiveShadow position={[0,.035,side*6.8]}><boxGeometry args={[15.7,.52,2.1]}/><meshStandardMaterial color="#b7b9a7"/></mesh>
    <mesh receiveShadow position={[side*6.8,.035,0]}><boxGeometry args={[2.1,.52,11.5]}/><meshStandardMaterial color="#b7b9a7"/></mesh>
  </group>)}</group>;
}

export function CameraControls({ reset, onUnavailable, onRestored, followTile }: { reset: number; onUnavailable: () => void; onRestored?: () => void; followTile?: number }): null {
  const { camera, gl } = useThree();
  const controlsRef = useRef<OrbitControls | null>(null);
  const desiredTarget = useMemo(() => { const [x,z] = tilePosition(followTile ?? 0); return followTile === undefined ? new Vector3() : new Vector3(x*.7,0,z*.7); },[followTile]);
  ...
  }, [camera, gl, reset, onUnavailable, onRestored]);
  return null;
}
...
function TileModel({ tile, ownerIndex, mapId, hideBuilding = false }: { tile: Tile; ownerIndex: number; mapId: string; hideBuilding?: boolean }): JSX.Element {
  ...
  const color = isProperty ? tileColors[tile.index % tileColors.length] ?? "#d99a36" : "#f6ead2";
  return (
    <group position={[x, 0, z]} rotation={[0, rotation, 0]}>
      <mesh receiveShadow castShadow position={[0, 0.15, 0]}><boxGeometry args={[1.65, 0.3, 1.15]} /><meshStandardMaterial color={color} roughness={0.65} /></mesh>
      ...
      {ownerIndex >= 0 ? <mesh position={[0.65, 0.4, -0.45]} rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[0.13, ownerIndex + 3]} /><meshStandardMaterial color={playerColors[ownerIndex]} /></mesh> : null}
    </group>
  );
}
...
function TileMarker({ type }: { type: Tile["type"] }): JSX.Element {
  const markerColor = type === "start" ? "#35bf7a" : type === "challenge" ? "#9257d4" : "#f49a45";
  return <mesh castShadow position={[0, 0.48, 0]}><cylinderGeometry args={[0.28, 0.38, 0.35, 24]} /><meshStandardMaterial color={markerColor} emissive={markerColor} emissiveIntensity={0.15} /></mesh>;
}
...
function Die({ value, nonce, reducedMotion }: { value: number; nonce: number; reducedMotion: boolean }): JSX.Element {
  ...
  const dots = useMemo(() => {
    const parts = faces.flatMap((face) => pipPositions(face.value).map(([x,y]) => new CircleGeometry(.065,12).translate(x,y,0).rotateX(face.rotation[0]).rotateY(face.rotation[1]).rotateZ(face.rotation[2]).translate(...face.position)));
    const merged = mergeGeometries(parts, false)!; parts.forEach((part) => part.dispose()); return merged;
  }, []);
  useEffect(() => () => dots.dispose(), [dots]);
  return <group ref={ref}>
    <mesh castShadow><boxGeometry args={[1, 1, 1]} /><meshStandardMaterial color="#fff9ee" roughness={0.3} /></mesh>
    <mesh geometry={dots}><meshBasicMaterial color="#23334a" /></mesh>
  </group>;
}
