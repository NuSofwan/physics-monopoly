  ...
  return (
    <div className="panel rounded-lg p-4">
      <div className="flex items-center justify-between">
        <h3 className="font-black">ทรัพย์สิน{owned.length > 0 ? ` (${owned.length})` : ""}</h3>
        <Landmark className="h-4 w-4 text-slate-500" />
      </div>
      <div className="mt-3 grid max-h-64 gap-2 overflow-auto">
        {owned.length === 0 ? <p className="rounded-lg bg-white px-3 py-2 text-sm text-slate-500">ยังไม่มีทรัพย์สิน</p> : null}
        {owned.map((tile) => (
          <div key={tile.index} className="rounded-lg bg-white p-3">
            <div className="flex items-start justify-between gap-2">
              <div>
                <p className="font-black">{tile.name}</p>
                <p className="text-xs text-slate-500">ระดับ {tile.level ?? 0} · ค่าเช่า ฿{tile.rentByLevel?.[tile.level ?? 0]?.toLocaleString("th-TH")}</p>
              </div>
            ...
            {!state.classroomMode ? <button className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-slate-100 px-3 py-2 text-xs font-black text-ink" onClick={() => sendSellProperty(tile.index)}>
              <Banknote className="h-4 w-4" /> ขาย ฿{mortgageValue(tile).toLocaleString("th-TH")}
            </button> : null}
          </div>
        ))}
      </div>
