  ...
  return (
    <div className={`panel rounded-lg p-3 transition ${active ? "ring-4 ring-gold" : ""} ${player.bankrupt ? "opacity-45" : ""}`}>
      <div className="flex items-center gap-3">
        <div className="grid h-11 w-11 place-items-center rounded-lg bg-ink text-sm font-black uppercase text-white">{player.avatar.slice(0, 1)}</div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
        ...
        <span className="rounded-md bg-white px-2 py-1 font-bold">มูลค่า ฿{calculateNetWorth(state, player.id).toLocaleString("th-TH")}</span>
      </div>
