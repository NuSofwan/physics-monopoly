...
const sourcePaths=["client/src/game3d/CityKit.tsx","client/src/game3d/AvatarFigure.tsx","client/src/game3d/diceGeometry.ts","client/src/game3d/Board3D.tsx","shared/src/locations.ts","shared/src/avatars.ts","shared/src/appearance.ts"];
...
const assets=[
  ...locations.map(city=>({id:`city:${city.id}`,path:sourcePaths[0],selector:city.id,kind:"city-kit",levels:[0,1,2,3],variants:2,clips:[],textures:[],triangles:{environment:measured.triangles[`city:${city.id}`],buildings:[0,1,2,3].map(level=>[0,1].map(variant=>measured.triangles[`building:${city.id}:${level}:${variant}`]))},readiness:"review_required"})),
