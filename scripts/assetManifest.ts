...
import { locations,avatarPresets,baseTiles } from "../shared/src/index";
const sourcePaths=["client/src/game3d/CityKit.tsx","client/src/game3d/AvatarFigure.tsx","client/src/game3d/diceGeometry.ts","client/src/game3d/Board3D.tsx","shared/src/locations.ts","shared/src/avatars.ts","shared/src/appearance.ts","client/src/game3d/SurfaceTextures.ts","client/src/game3d/PropertyArchitecture.tsx","shared/src/boardConfig.ts"];
...
const assets=[
  ...locations.map(city=>({id:`city:${city.id}`,path:sourcePaths[0],selector:city.id,kind:"city-kit",levels:[0,1,2,3],variants:2,propertyTileIndices:propertyTiles,clips:[],textures:["client/public/assets/textures/limestone-plaster.png",...(city.id==="bangkok"?["client/public/assets/textures/terracotta-roof.png"]:[]),"procedural:stone","procedural:roof","procedural:wood","procedural:terrain","procedural:water"],triangles:{environment:measured.triangles[`city:${city.id}`],buildings:[0,1,2,3].map(level=>[0,1].map(variant=>measured.triangles[`building:${city.id}:${level}:${variant}`])),properties:propertyTiles.map(tileIndex=>measured.triangles[`property:${city.id}:${tileIndex}`])},readiness:"review_required"})),
